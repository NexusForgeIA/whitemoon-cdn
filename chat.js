...
(function(){
  ...
  function verifyToken(){
    verifyEdge(token).then(function(res){
      ...
      if(res){
        // Respuesta recibida y es que NO: licencia pausada por impago, token
        // desconocido o dominio no autorizado. Se corta aquí, SIN fallback.
        console.warn('[WM-CHAT] Licencia no activa');
        return;
      }
      // Sin respuesta de la Edge Function → fallback a licenses.json, para no
      // dejar sin servicio a quien sí paga porque Supabase tenga un hipo.
      fetchLicensesJson().then(function(licenses){
