/**
 * WHITEMOON CHATBOT IA — Widget con Licencia
 * Insertar en web del cliente:
 * <script src="https://TUUSUARIO.github.io/whitemoon-cdn/chat.js" data-token="WM-XXXX-NNN"></script>
...
(function(){
  ...
  function initChat(el,lic){
    var cfg={
      ...
      color:el.getAttribute('data-color')||'#7c3aed',
      phone:el.getAttribute('data-phone')||lic.phone||'',
      services:(el.getAttribute('data-services')||'Información,Pedir cita,Contactar').split(',').map(function(s){return s.trim()}),
      botName:el.getAttribute('data-bot-name')||'Asistente',
      position:el.getAttribute('data-position')||'right',
      greeting:el.getAttribute('data-greeting')||'',
      calendar:el.getAttribute('data-calendar')!=='false'
    };
    ...
    window._wmS={step:0,name:'',tel:'',service:'',date:'',time:'',cfg:cfg};
    ...
    window.wmChatBtn=function(el){var s=window._wmS;wmAddMsg(el.textContent,true);var b=document.getElementById('wmInitBtns');if(b)b.remove();s.service=el.textContent;s.step=1;setTimeout(function(){wmAddMsg('Para darte la mejor atenci\u00f3n sobre '+s.service+', \u00bfme dices tu nombre y apellidos?',false)},800)};

    window.wmSend=function(){var inp=document.getElementById('wmInput');var m=inp.value.trim();if(!m)return;inp.value='';wmAddMsg(m,true);var s=window._wmS;
      if(s.step===0){s.service=m;s.step=1;setTimeout(function(){wmAddMsg('Para ayudarte, \u00bfme dices tu nombre y apellidos?',false)},800)}
      else if(s.step===1){s.name=m;s.step=2;setTimeout(function(){wmAddMsg('Perfecto '+s.name.split(' ')[0]+'. \u00bfY un tel\u00e9fono m\u00f3vil para contactarte?',false)},800)}
      else if(s.step===2){s.tel=m;s.step=3;if(s.cfg.calendar){setTimeout(function(){wmAddMsg('Genial. Elige el d\u00eda que prefieras:',false);setTimeout(wmShowCal,400)},800)}else{setTimeout(function(){wmAddMsg('\u00bfPara qu\u00e9 d\u00eda te vendr\u00eda bien?',false)},800)}}
      else if(s.step===3){s.date=m;s.step=99;setTimeout(function(){wmAddMsg('Perfecto '+s.name.split(' ')[0]+'. He tomado nota:\n\u2022 Nombre: '+s.name+'\n\u2022 Tel\u00e9fono: '+s.tel+'\n\u2022 Servicio: '+s.service+'\n\u2022 Cita: '+m+'\n\nNuestro equipo revisar\u00e1 los datos y confirmar\u00e1 la cita por WhatsApp. Si no hubiera disponibilidad, te propondremos la alternativa m\u00e1s cercana. \u00a1Saludos y gracias! \ud83d\ude0a',false)},1000)}
      else{setTimeout(function(){wmAddMsg('Nuestro equipo te contactar\u00e1 por WhatsApp para confirmar. \u00a1Saludos y gracias! \ud83d\ude0a',false)},800)}
    };
    ...
    window.wmPickTime=function(el,t){el.parentElement.querySelectorAll('.wm-time').forEach(function(x){x.classList.remove('wm-sel')});el.classList.add('wm-sel');var s=window._wmS;s.time=t;s.step=99;wmAddMsg(s.date+' a las '+t,true);setTimeout(function(){wmAddMsg('Perfecto '+s.name.split(' ')[0]+'. He tomado nota:\n\u2022 Nombre: '+s.name+'\n\u2022 Tel\u00e9fono: '+s.tel+'\n\u2022 Servicio: '+s.service+'\n\u2022 Fecha: '+s.date+'\n\u2022 Hora: '+s.time+'\n\nNuestro equipo revisar\u00e1 los datos y confirmar\u00e1 la cita por WhatsApp. Si no hubiera disponibilidad, te propondremos la alternativa m\u00e1s cercana. \u00a1Saludos y gracias! \ud83d\ude0a',false)},1000)};
  }
})();
